...
const allowedOrigins = [
  ...
  'http://13.232.187.62:9065',
  'http://13.232.187.62:9066',
  'http://13.232.187.62:3097'
];
    ...
    try {
      await pool.query(`
        CREATE TABLE IF NOT EXISTS leaves (
          ...
          CONSTRAINT valid_email CHECK (email ~ '^[a-zA-Z0-9._%+-]+@[a-zA-Z]{2,15}\\.[a-zA-Z]{2,3}$')
        );
      `);
...
const validators = {
  ...
  email: (email) => /^[a-zA-Z0-9._%+-]+@[a-zA-Z]{2,15}\.[a-zA-Z]{2,3}$/.test(email),
  name: (name) => /^[A-Za-z]+(?:\.[A-Za-z]+)*(?: [A-Za-z]+)*(?:\.[A-Za-z]+){0,3}$/.test(name),
  leaveType: (type) => ['sick', 'casual', 'earned'].includes(type),
...
app.get('/api/leave-stats/:emp_id', async (req, res) => {
  try {
    ...
    const allocatedLeaves = 30; // This could come from another table
    const result = await pool.query(
    ...
    res.json({ 
      ...
      usedLeaves, 
      remainingLeaves,
      utilizationPercentage: (usedLeaves / allocatedLeaves * 100).toFixed(2)
    });
  } catch (error) {
...
async function startServer() {
  ...
  - GET /api/leaves - Get all leaves (optional ?status= filter)
  - GET /api/leaves/:emp_id - Get leaves for specific employee
  - PUT /api/leaves/:id - Update leave status
  - GET /api/leave-stats/:emp_id - Get leave statistics
